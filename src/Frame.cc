...
std::vector<cv::Point2f> Frame::Calc2DCoordinates(const int chk_num_h,
                                                  const int chk_num_w,
                                                  const int count)
{
  std::vector<cv::Point2f> reg_cor;
  cv::TermCriteria criteria = cv::TermCriteria(
      cv::TermCriteria::MAX_ITER + cv::TermCriteria::EPS, 30, 0.001);

  bool cbfound =
      cv::findChessboardCorners(mImGray,
                                cv::Size(chk_num_w - 1, chk_num_h - 1),
                                reg_cor,
                                cv::CALIB_CB_NORMALIZE_IMAGE);
  if (cbfound)
  {
    cv::cornerSubPix(mImGray,
                     reg_cor,
                     cv::Size(chk_num_w - 1, chk_num_h - 1),
                     cv::Size(-1, -1),
                     criteria);
    cv::drawChessboardCorners(
        mImGray, cv::Size(chk_num_w - 1, chk_num_h - 1), reg_cor, cbfound);
    cv::imwrite("check_" + std::to_string(count) + ".jpg", mImGray);
    std::cout << "Checkerboard " << count << " found" << std::endl;
    return reg_cor;
  }
  else
  {
    std::cout << "Checkerboard " << count << " not found" << std::endl;
  }
  // cv::imshow("cb", mImGray);
  // cv::waitKey(0);
