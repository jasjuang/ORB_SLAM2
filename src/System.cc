...
// This function gives out the vector of poses for the keyframes and information
// about which frames became keyframes in the first place.
void System::GetAllPoses(std::vector<int> &kf_indices,
                         std::vector<cv::Mat> &kf_poses)
{
  cout << "Outputting a vector of poses and the indices they correspond to"
       << endl;
  vector<KeyFrame *> vpKFs = mpMap->GetAllKeyFrames();
  sort(vpKFs.begin(), vpKFs.end(), KeyFrame::lId);
  ...
  for (size_t i = 0; i < vpKFs.size(); i++)
  {
    ...
    pKF->SetPose(Two *
                 pKF->GetPose());  // to bring to global frame of reference

    cv::Mat scale_corrected_pose = pKF->GetPose().clone();
    scale_corrected_pose.at<float>(0, 3) =
        scale_corrected_pose.at<float>(0, 3) * mpTracker->mScale;
    scale_corrected_pose.at<float>(1, 3) =
        scale_corrected_pose.at<float>(1, 3) * mpTracker->mScale;
    scale_corrected_pose.at<float>(2, 3) =
        scale_corrected_pose.at<float>(2, 3) * mpTracker->mScale;
    kf_poses.push_back(scale_corrected_pose.clone());
    kf_indices.push_back(pKF->mnFrameId);
    cout << "Index of the frame" << pKF->mnFrameId << endl;
  }
}
